...
- **ジャンル別に表示**：CSV の `category` 列ごと（基本の返事・お礼・あいさつ…）に見出しを付けて並べます。見出しの「このジャンルを選ぶ」でまとめて選択できます。チェックを外すと番号順の一覧に戻ります
- **販売状況**：スタンプを選んで「申請中にする」「販売中にする」を押すと、一覧に印が付きます（「未販売に戻す」で外せます）。「表示」で未販売だけ・販売中だけに絞り込めます。LINE 側の状況は自動では取得できないので、自分で付ける印です。保存先は `data/sales.json`（Git の管理対象外）
...
## 11-2. ローカルAI（文章づくりの補助・無料）

自分のPCで動く LLM（ik_llama.cpp / llama.cpp の `llama-server`、Ollama など、OpenAI 互換の
`/v1/chat/completions` を持つサーバー）を使って、次の文章を作れます。**料金はかかりません。画像は作りません。**

| 場所 | ボタン | できること |
|---|---|---|
| はじめに ＞ キャラクターの特徴 | ローカルAIで整える | 説明文を、絵にしやすい具体的な文章に書き直す（1行目の「何のキャラか」は必ず残す） |
| セリフ編集 | 案を出す | テーマ（例: 推し活）に合うセリフ・ポーズ・表情・カテゴリの案を出し、選んだものを表に追加 |
| セリフ編集 | ポーズ・表情の空欄をAIで埋める | 空欄の行だけ、セリフに合うポーズ・表情を入れる |
| 検証・出力 ＞ タイトル・説明文 | ローカルAIで作る | 自然な文章で案を作る。LINE のルールに合わない項目は理由を伝えて書き直させる |

- どれも**保存はしません**。内容を確認して、それぞれの「保存」ボタンで確定してください
- 接続先は `config/sticker_config.yaml` の `llm:` で変えられます（既定は `http://127.0.0.1:11434/v1`）
- サーバーが止まっていると、ボタンの横に「ローカルAI: 未起動」と出ます。ほかの機能はそのまま使えます
- CPU だけで動かすと時間がかかります（目安: 4コアで Gemma 4 E4B、セリフ10件で約1分、タイトル・説明文で約20秒）
